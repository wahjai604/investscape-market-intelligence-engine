# E87 Phase 7 — Production Hardening & End-to-End Contract Validation
...
wires multiple phases together per test — cited E86-shaped observations
(`CRECitedObservation` fixtures) → Phase 2 `evaluateComparability` → Phase 3
`buildCapRateBenchmark` → (Phase 4 `deriveTransactionCapRate` /
`toE87CandidateInput` where relevant) → Phase 5 `resolveCapRateBenchmark`
...
9. **Large-pool determinism** — a 150-synthetic-candidate pool, run through
   ...
   byte-identical (`JSON.stringify`-equal) `E87PipelineResult` output, and the
...
|---|---|---|
| `E87ComparabilityRequest`, `E87CandidateInput`, `E87ComparabilityCandidate`, `E87ComparabilityResult`, `E87ComparabilityDimensions`, `E87DimensionResult`, `E87MatchLevel`, `E87InclusionDecision`, `E87ExclusionReasonCode` | comparability-types.ts | **Intentional public API** — the Phase 2 request/response contract. |
...
| `E87Representation`, `E87SourceHierarchyTier`, `SOURCE_HIERARCHY_RANK`, `E87ConfidenceTier`, `CONFIDENCE_RANK`, `floorConfidence`, `DispersionPolicy`, `DEFAULT_DISPERSION_POLICY`, `E87DispersionTier`, `E87Dispersion`, `E87BenchmarkGapReasonCode`, `E87EnrichedCandidate`, `E87BenchmarkAuditEntry`, `E87BenchmarkAudit`, `E87Benchmark`, `E87BenchmarkGap`, `E87BenchmarkResult`, `E87BenchmarkOptions` | consensus-types.ts | **Intentional public API** — the Phase 3 result contract, plus small reusable primitives (`floorConfidence`, `CONFIDENCE_RANK`) that Phase 7's own e2e tests rely on to assert floor behavior from outside the module. |
...
| `capRateDifferenceBps`, `classifyReconciliationTier`, `deriveTransactionCapRate`, `deriveTransactionCapRateBatch`, `toE87CandidateInput` | transaction-derivation.ts | **Intentional public API** — `deriveTransactionCapRate`/`Batch` are the Phase 4 entry points, `toE87CandidateInput` is the documented Phase 4→Phase 2 bridge used by Phase 5 itself, and `capRateDifferenceBps`/`classifyReconciliationTier` are exercised directly by `transaction-derivation.test.ts`. |
| `E87BenchmarkOrigin`, `E87PipelineRequest`, `E87OverrideLayer`, `E87PipelineSuccessResult`, `E87PipelineDataGapResult`, `E87PipelineSuccess`, `E87PipelineDataGap`, `E87PipelineUserOverridden`, `E87PipelineResult` | pipeline-types.ts | **Intentional public API** — the Phase 5 request/result contract. |
| `resolveCapRateBenchmark` | pipeline.ts | **Intentional public API** — the sole Phase 5 entry point. |
| `classifyOrigin`, `toE86BenchmarkResponseStub` | pipeline.ts | **Internal implementation detail — already correctly not exported.** Both are plain (non-`export`) functions local to `pipeline.ts`; they are not re-exported by the barrel and were confirmed absent from every import in `__tests__/cap-rate-engine/`. No change needed. |
| `E87ScenarioProvenance`, `E87ScenarioType`, `E87ScenarioBasis`, `E87Scenario`, `E87EvidenceSupportedRange`, `E87SensitivityDeltaInput`, `E87SensitivityRequest`, `DefaultSensitivityPolicy`, `DEFAULT_SENSITIVITY_POLICY`, `E87ScenarioGapReasonCode`, `E87ScenarioError`, `E87ScenarioDataGap`, `E87ScenarioSetSuccess`, `E87HypotheticalScenarioSet`, `E87ScenarioResult` | scenario-types.ts | **Intentional public API** — the Phase 6 contract, including the versioned, clearly-labeled `DEFAULT_SENSITIVITY_POLICY` opt-in convenience. |
...
description (`classifyOrigin`, `toE86BenchmarkResponseStub`) were already
...
- **`user-override.ts`**: E87 calls E86's real `createUserOverride` /
  `resolveBenchmark` verbatim via a private adapter
  (`toE86BenchmarkResponseStub` in `pipeline.ts`) rather than reimplementing
  "an override always wins." This is the one place E87 depends on E86 logic
  beyond types, and it is read-only — E87 constructs an honest, minimal stub
  of E86's `CREBenchmarkResponse` shape and never fabricates fields E86 would
...
- **`benchmark-selection.ts` / `qualification.ts`**: conceptually adjacent to
  ...
  E87 maintains its own comparability/consensus logic so E86 stays frozen at
  v1.0 and so E87's cap-rate-specific rules (capRateType family exactness,
  representation transforms, source hierarchy) don't have to be shoehorned
  into E86's more general axis-qualification model. This is an accepted,
...
- **`observation-lifecycle.ts` (`CREPresentationFreshness`)**: E87 never
...
- `DEFAULT_SENSITIVITY_POLICY` (`scenario-types.ts`):
  `{ version: "E87-phase6-provisional-v1", deltasBps: [-50, -25, 0, 25, 50]
...
- Duplicate-observation handling: Phase 2/3 do not deduplicate structurally
  ...
  changed. Whether a caller-side or E87-side dedup step should exist is a
  ...
  outside E87's contract, in which case no engine-level fix is owed) — flagged
...
adversarial), `tsc --noEmit` is clean, no E86 file was touched, no Phase 1-6
...
explicitly accepted) before treating E87 as fully closed.
