# E69 Phase 5 — Unified Cap-Rate Benchmark Output / End-to-End Integration
...
request -> candidate pool (+ transaction-derived merge)
  ...
  -> one deterministic E69PipelineResult
```
...
No Phase 2/3/4 file's logic was changed. No E68 file was changed.

## 3. The unified output contract

`E69PipelineResult` (in `pipeline-types.ts`) is a discriminated union on
`pipelineStatus`:

- **`"success"`** (`E69PipelineSuccess`) — a benchmark was produced. Carries
...
- **`"user_overridden"`** (`E69PipelineUserOverridden`) — a user override is
...
- **`"data_gap"`** (`E69PipelineDataGap`) — no benchmark could be produced.
  Carries the **entire, unflattened** Phase 3 `E69BenchmarkGap` plus its
  audit (`result`).
...
distinct from `E69PipelineSuccess`+`origin`. Phase 3's `E69Benchmark` shape
...
1. Convert every `transactionDerivedObservations` entry via Phase 4's
   `toE69CandidateInput` (unmodified) and merge the results into
...
6. If `request.userOverride` is present: build a minimal
   `CREBenchmarkResponse` stub (see §6), call E68's real
...
second** (`transactionDerivedObservations` on `E69PipelineRequest`) because:

- It keeps `candidatePool` a pure `E69CandidateInput[]` (already-resolved E68
...
- A caller with raw Phase 4 output never needs to learn or duplicate
  `toE69CandidateInput`'s conversion — the pipeline does it, using Phase 4's
...
## 5. E68 components reused

- `createUserOverride` (`src/cre-intelligence/user-override.ts`) — called
  with `overrideValue`, `overrideReason`, `originalE68Value` (the underlying
  Phase 3 benchmark's value when the underlying result was a success, else
  `undefined` — never fabricated), `originalE68Identity` (from the stub
...
- `resolveBenchmark` (`src/cre-intelligence/user-override.ts`) — called with
  ...
  confirm "override always wins when present" — the one line of E68 logic
...
- Indirectly, everything Phase 2/3/4 already reuse from E68:
...
### §6 — Why a stub `CREBenchmarkResponse` is built, and why this doesn't modify E68

`resolveBenchmark`'s real signature is
`(e68: CREBenchmarkResponse | undefined, override: UserOverride | undefined) => ResolvedBenchmark`.
E69's Phase 3 result (`E69BenchmarkResult`) is a different, richer shape than
E68's `CREBenchmarkResponse` — E68's contract was frozen before E69 existed
and was never designed to carry E69's two-axis confidence model, dispersion,
or comparability dimensions. Rather than changing E68's `resolveBenchmark` to
accept E69's shape (forbidden — E68 is frozen v1.0), `pipeline.ts`'s private
`toE68BenchmarkResponseStub` builds the minimal `CREBenchmarkResponse` that
`resolveBenchmark` actually inspects (`status`, used only to distinguish
"available" from "DATA_GAP" — `resolveBenchmark`'s own gate for whether `e68`
would have been used absent an override) plus the identity fields
`createUserOverride`'s `originalE68Identity` wants to preserve for display.
This stub is **never returned to a caller** — it is a private adapter, not a
new public contract, and it neither reads from nor writes to any E68 file.
...
- `toE69CandidateInput` (Phase 4, `transaction-derivation.ts`).
- Types: `E69CandidateInput`, `E69ComparabilityRequest` (Phase 2);
  `E69BenchmarkOptions`, `E69BenchmarkResult` (Phase 3);
...
`E69PipelineSuccess.result`/`E69PipelineUserOverridden.underlying`. They are
...
- `request.userOverride` is optional. When present, it **always wins**,
  mirroring E68's `resolveBenchmark` contract exactly (verified, not
  reimplemented — see §4 step 6 and §6).
- The override never mutates or discards the underlying benchmark/DATA_GAP:
  `E69PipelineUserOverridden.underlying` is the full, untouched Phase 3
...
- No new user-identity/authentication concept was introduced: the pipeline
  ...
  required by E68's existing contract, so none was invented here.

## 11. DATA_GAP requirements

Every DATA_GAP result (`E69PipelineDataGap`, or `E69PipelineUserOverridden`
wrapping a data_gap `underlying`) carries Phase 3's full
`E69BenchmarkGap`/`E69BenchmarkAudit`, which together already answer every
required question:

- **Why no benchmark can be produced** — `gap.reasonCode` (one of Phase 3's
  seven `E69BenchmarkGapReasonCode`s) plus `gap.explanation`.
...
- **Freshness state** — preserved per-candidate on the audit entries that
  ...
  `minFreshness` gate.
- **Whether a user override exists** — `E69PipelineUserOverridden` is a
...
silently inherited. `E69PipelineRequest.benchmarkOptions` still allows a
...
boundary concern rather than modify E68. Observations:

- `qualification.ts`'s `qualifyCapRateObservation` and `benchmark-selection.ts`'s
  `matchesIdentity` implement conceptually similar ideas to E69 Phase 2/3
  (floor-combination of match dimensions; "only filter on what the request
  specifies"). E69 Phase 2/3 **intentionally re-implements these ideas in its
  own module** rather than calling E68's versions directly, because E68's
  versions are scoped to E68's own benchmark-identity model
  (`BenchmarkIdentity`, E68's specific axis set) and are frozen — extending
  them to E69's richer requirements (8 dimensions, transaction-derived
  representation, two-axis confidence) would require modifying frozen E68
...
- No analytical defect was found in either E68 file during this review; the
...
`audit.reconciliation` keeps both, surfaced unmodified); never overwrites E69
...
- The `toE68BenchmarkResponseStub` adapter populates `identity.city` as
  `geography.city ?? geography.metro ?? geography.country` — a reasonable
  best-effort label for a value E68's `resolveBenchmark` does not actually
