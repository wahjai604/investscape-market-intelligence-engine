...
- `statistical-risk/phase2-contracts.ts` — `MonteCarloRequest`, `ProbabilityResult`, `runMonteCarloSimulation()`; `PortfolioCovarianceRequest`, `PortfolioCovarianceResult`, `runPortfolioCovarianceAnalysis()`. Pure math, no `MarketObservation` dependency, stays in the zero-dependency tree.
- `market-intelligence/phase2-contracts.ts` — `ForecastRequest`, `ForecastPoint`, `ForecastResult`, `runForecast()`; `RegressionRequest`, `RegressionCoefficient`, `ModelDiagnostics`, `RegressionResult`, `runRegression()`; `BacktestRequest`, `BacktestDiagnostics`, `runBacktest()`. All six of these depend on `MarketObservation` per their request/actuals shapes — that dependency is exactly why this file lives in `market-intelligence/` rather than `statistical-risk/`.

All `run*()` functions throw immediately (`"Phase 2 not implemented..."`, or `"Phase 2+ not implemented..."` for the one exception below) rather than returning a fabricated result — there is no "close enough" placeholder for a forecast, regression fit, backtest, Monte Carlo simulation, or covariance matrix. When Phase 2 implementation begins, expect these shapes to be revisited against real modeling requirements rather than treated as frozen — the doc comments say so explicitly.
...
### Regression / ModelDiagnostics — a documented judgment call, not a spec transcription

Unlike `ForecastRequest`/`MonteCarloRequest`, the master spec gives a *category* for this ("regression models"; Section 9's intent implies validating a fitted model), not a code block. `ModelDiagnostics`' exact field names (`rSquared`, `adjustedRSquared`, `standardError`, `sampleSize`, `coefficients: RegressionCoefficient[]`, `residualDiagnostics`) are a judgment call, chosen to follow standard regression-diagnostic vocabulary rather than invent unfamiliar terminology — the minimum a caller would need to judge whether a fitted model is trustworthy, not an exhaustive list of everything a regression library could produce. `RegressionCoefficient` carries `estimate`, `standardError?`, `confidenceInterval?`, and `pValue?` per coefficient (keyed by predictor name, or `"intercept"`).

`RegressionRequest.target` and `.predictors` (a `Record<string, MarketObservation[]>`, one series per named predictor) are `MarketObservation[]`-typed for the same reason `ForecastRequest.series` is — the data being regressed is real market data with real provenance, not an opaque number array.

### Back-testing diagnostics — a separate interface, not a `ModelDiagnostics` field

`BacktestRequest`/`BacktestDiagnostics` (`meanAbsoluteError`, `rootMeanSquaredError`, `meanAbsolutePercentageError`, `intervalCoverage?`, `sampleSize`, `issues`) follow the parent Master Implementation Blueprint's own release-gate (MAE/RMSE/MAPE/coverage) language. Deliberately kept as its own interface rather than folded into `ModelDiagnostics`: back-testing evaluates a model's *predictions* against held-out actuals over time, which applies equally to a `ForecastResult` or a `RegressionResult` — coupling it to `ModelDiagnostics` specifically would have made it regression-only for no real reason. `BacktestRequest.predicted` is a plain `{period, estimate, lower?, upper?}` array (matching `ForecastPoint`'s fields structurally) rather than importing `ForecastPoint` directly, so a regression fit's coefficients can be turned into the same shape without forcing a dependency between the two result types.

### Portfolio covariance/correlation risk — the most speculative scaffold, and the one placement exception

`PortfolioCovarianceRequest`/`PortfolioCovarianceResult` is the one contract in this package that plausibly *could* have followed Regression/Backtest's `MarketObservation`-typed pattern but deliberately didn't. Two reasons, both in the file's own doc comment: (1) the master spec's own Phase Scope Matrix marks portfolio covariance **"PHASE 2+"** — one notch further out than the rest of Phase 2 — so locking in a `MarketObservation`-shaped contract this early would be premature; (2) the underlying math (a covariance/correlation matrix) is a direct n-ary generalization of `statistical-risk/correlation.ts`'s `pearsonCorrelation()` — already pure math in the *same* tree — so keeping it beside that precedent is more consistent than domain-coupling it. `series: Record<string, number[]>` (named return/value series, no alignment contract defined yet) and `population?: boolean` (mirroring `dispersion.ts`'s own sample/population option) are both intentionally loose rather than over-specified. `runPortfolioCovarianceAnalysis()` throws with a distinct `"Phase 2+ not implemented..."` message (not the plain `"Phase 2 not implemented..."` the other five `run*()` functions use) so the extra distance from being built stays visible at the call site, not just in a doc comment.
