/**
 * InvestScape™ E69 Phase 5 — Unified Cap-Rate Benchmark Output: types.
 ...
 *
 * E68 reuse: `UserOverride` (src/cre-intelligence/benchmark-types.ts) is
 ...
 * reimplemented — see pipeline.ts's `toE68BenchmarkResponseStub` for how an
 * E69 result is adapted to fit E68's `CREBenchmarkResponse` shape without
 * modifying E68.
 */
import type { CREGeography } from "../cre-intelligence/types";
import type { E69CandidateInput, E69ComparabilityRequest } from "./comparability-types";
import type { E69BenchmarkOptions, E69BenchmarkResult } from "./consensus-types";
 ...
 */
export type E69BenchmarkOrigin = "publisher_survey" | "transaction_derived" | "consensus";
 ...
 * each one via Phase 4's own `toE69CandidateInput` and merges the result into
 ...
 * already resolved E68 observations, while still letting a caller who only
 * has raw transaction inputs plug in Phase 4's output without first learning
 * `toE69CandidateInput`'s exact shape.
 */
export interface E69PipelineRequest {
  comparability: E69ComparabilityRequest;
  candidatePool: readonly E69CandidateInput[];
  transactionDerivedObservations?: readonly CRETransactionDerivedObservation[];
  benchmarkOptions?: E69BenchmarkOptions;
  /**
   * When supplied, always wins (mirrors E68's `resolveBenchmark`: "an
...
export interface E69OverrideLayer {
  overrideValue: number;
  overrideReason: string;
...
export type E69PipelineSuccessResult = Extract<E69BenchmarkResult, { status: "success" }>;
export type E69PipelineDataGapResult = Extract<E69BenchmarkResult, { status: "data_gap" }>;

/** A benchmark was produced, and no override was supplied (or none was needed to determine the active value). */
export interface E69PipelineSuccess {
  pipelineStatus: "success";
  origin: E69BenchmarkOrigin;
  /** The full, unflattened Phase 3 success result: benchmark, both confidence axes, contributing observations, dispersion, audit. */
  result: E69PipelineSuccessResult;
...
export interface E69PipelineDataGap {
  ...
  result: E69PipelineDataGapResult;
  requestedGeography: CREGeography;
}
 ...
 */
export interface E69PipelineUserOverridden {
  ...
  underlying: E69PipelineSuccessResult | E69PipelineDataGapResult;
  override: E69OverrideLayer;
  requestedGeography: CREGeography;
}

export type E69PipelineResult = E69PipelineSuccess | E69PipelineDataGap | E69PipelineUserOverridden;
