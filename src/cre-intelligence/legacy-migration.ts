/**
 * InvestScape™ E68 Phase 5 — legacy benchmark protection (Part 10).
 ...
 * docs/E68-phase5-application-integration.md, Part 1). This file therefore
 * cannot migrate real data; it defines the audit function the WeWeb/frontend
 * repository should run against its own constants once E68 is wired in
 * there, so an unverified number never gets treated as if E68 produced it.
 */
...
export interface LegacyBenchmarkEntry {
  ...
  source?: string;
  /** Whether an E68 observation has been positively matched to this key. */
  hasE68Mapping: boolean;
}
 ...
 * it. Either way, no legacy value is ever returned relabeled as E68-sourced.
 */
export function auditLegacyBenchmark(entry: LegacyBenchmarkEntry, options: { nullify?: boolean } = {}): LegacyBenchmarkAudit {
  ...
  const hasValidMapping = entry.hasE68Mapping;
  ...
  if (hasSource && hasValidMapping) {
    return {
      ...
      reason: "Has both a stated source and a matched E68 observation, but has not itself been re-derived through E68's qualification pipeline — retained as unverified, not promoted to E68-sourced automatically.",
    };
  }
  ...
  if (!hasValidMapping) reasonParts.push("no matching E68 observation");
  ...
  return {
    ...
    hasValidMapping,
    classification,
    reason: `${reasonParts.join(" and ")} — this value must not be presented as E68-sourced.`,
  };
}
